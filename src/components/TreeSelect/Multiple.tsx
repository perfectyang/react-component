...
const Index: React.FC<IProps> = ({ options, values }) => {
  ...
  const renderLeftItem = (options: ITree[], i) => {
    return options?.map((option, idx) => {
      const bool = value.includes(option.value);
      return (
        <div
          key={idx}
          onClick={() =>
        ...
        >
          <input type="checkbox" checked={bool} /> {option.label}
        </div>
      );
    });
  };
  ...
  const renderItems = (options, i) => {
    const isLeft = options?.every((e) => e.isLastLeft);
    if (isLeft) {
      return renderLeftItem(options, i);
    } else {
      return renderItem(options, i);
    }
  };
